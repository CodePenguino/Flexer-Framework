...
void ecs_init(uint32_t n, ...)
{
	...
	state.component_store.data_size_array = (size_t*)malloc(n * sizeof(size_t));
	state.component_store.data_offset_array = (size_t*)malloc(n * sizeof(size_t));
	...
	state.entity_store.mask_array = (uint32_t*)malloc(INITIAL_CAPACITY * sizeof(uint32_t));
	state.entity_store.flag_array = (uint32_t*)malloc(INITIAL_CAPACITY * sizeof(uint32_t));
}
...
Entity ecs_create()
{
	...
	else
	{
		...
		{
			uint32_t *new_flag_array = (uint32_t*)realloc(state.entity_store.flag_array, state.entity_store.cap * 2 * sizeof(uint32_t));
			uint32_t *new_mask_array = (uint32_t*)realloc(state.entity_store.mask_array, state.entity_store.cap * 2 * sizeof(uint32_t));
			void *new_data = realloc(state.component_store.data, state.component_store.cap * 2 * state.component_store.size);
			uint32_t *new_query_result_list = (uint32_t*)realloc(state.query_result.list, state.entity_store.cap * 2 * sizeof(uint32_t));
